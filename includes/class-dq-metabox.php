...
class DQ_Metabox {
    ...
    public static function render( $post ) {
        ...
        // Inside your existing metabox callback:
        $snap = get_post_meta( $post->ID, '_dq_gmail_snapshot', true );
        if ( $snap ) {
            echo '<h4>Invoice Email (Gmail)</h4>';
            echo '<p><strong>From:</strong> ' . esc_html( $snap['from'] ) . '</p>';
            echo '<p><strong>Subject:</strong> ' . esc_html( $snap['subject'] ) . '</p>';
            echo '<p><strong>Date:</strong> ' . esc_html( $snap['date'] ) . '</p>';
            echo '<p><strong>Snippet:</strong><br>' . esc_html( $snap['snippet'] ) . '</p>';
            echo '<p><a class="button button-secondary" target="_blank" rel="noopener" href="'.esc_url($snap['open_url']).'">Open in Gmail</a></p>';
        }
        
          // Inside your metabox render method, near other buttons:
        wp_nonce_field('dq_fetch_gmail', 'dq_fetch_gmail_nonce');
        echo '<button type="button" class="button" id="dq-fetch-gmail">Fetch Invoice from Gmail</button>';
        echo '<span id="dq-fetch-gmail-status" style="margin-left:8px;"></span>';
        
        ?>
        <script>
        document.addEventListener('DOMContentLoaded', function(){
          var btn = document.getElementById('dq-fetch-gmail');
          if(!btn) return;
          btn.addEventListener('click', function(){
            var s = document.getElementById('dq-fetch-gmail-status');
            s.textContent = 'Fetching...';
            var data = new FormData();
            data.append('action','dq_fetch_gmail_invoice');
            data.append('post_id','<?php echo esc_js($post->ID); ?>');
            data.append('_ajax_nonce','<?php echo esc_js( wp_create_nonce('dq_fetch_gmail') ); ?>');
        
            fetch(ajaxurl, { method:'POST', credentials:'same-origin', body:data })
              .then(r=>r.json()).then(function(resp){
                if(resp.success){
                  s.innerHTML = 'Done. <a href="'+resp.data.file_url+'" target="_blank">View image</a>';
                } else {
                  s.textContent = 'Error: '+resp.data;
                }
              }).catch(function(e){
        ...
        wp_nonce_field('dq_fetch_gmail_emailshot', 'dq_fetch_gmail_emailshot_nonce');
        echo '<button type="button" class="button" id="dq-fetch-gmail-emailshot">Fetch Email Screenshot</button>';
        echo '<span id="dq-fetch-gmail-emailshot-status" style="margin-left:8px;"></span>';
        ?>
        <script>
        document.addEventListener('DOMContentLoaded', function(){
          var btn = document.getElementById('dq-fetch-gmail-emailshot');
          if(!btn) return;
          btn.addEventListener('click', function(){
            var s = document.getElementById('dq-fetch-gmail-emailshot-status');
            s.textContent = 'Rendering...';
            var data = new FormData();
            data.append('action','dq_fetch_gmail_emailshot');
            data.append('post_id','<?php echo esc_js($post->ID); ?>');
            data.append('_ajax_nonce','<?php echo esc_js( wp_create_nonce('dq_fetch_gmail_emailshot') ); ?>');
        
            fetch(ajaxurl, { method:'POST', credentials:'same-origin', body:data })
              .then(r=>r.json()).then(function(resp){
                if(resp.success){
                  s.innerHTML = 'Done. <a href="'+resp.data.file_url+'" target="_blank">Open image</a>';
                } else {
                  s.textContent = 'Error: '+resp.data;
                }
              }).catch(function(){
